...
| Encryption | ⚠️ | Transport: TLS via the Caddy overlay is now the documented default deploy path (`docs/OPERATIONS.md`), but still needs a real domain and so can't be unconditional — the no-TLS fallback still exists. At-rest: still genuinely missing — the underlying EBS volume isn't encrypted by default and no application-level encryption exists for uploads/vector_store. This half of the gap has no docs-only fix; it needs either an `--encrypted` EBS volume at instance-launch time (an AWS setting, not app code) or application-level encryption of files before they're written to disk (a real code change, not attempted — would need its own key-management story and a migration plan for existing on-disk data). |
